...
Shader "Unlit/BoxProjectedLightingV2"
{
    Properties
    {
            ...
            float4 fragment_forward_base(vertexToFragment i) : SV_Target
            {
                ...
                // Compute the direct bounce lighting term.
                    ...
                    for (int i = 1; i <= _BounceSamples; i++)
                    {
                        ...
                        // Looking only for intersections in the forward direction of the ray.
                        float3 vector_bounce_largestRayParameters = max(vector_worldCubeMax / vector_rayDirection, vector_worldCubeMin / vector_rayDirection);
                        ...
                        // Find the position of the intersection point. (This is the point where the light intersects with the box)
                        float3 vector_bounce_worldSpaceIntersectPosition = vector_worldSpaceIntersectPosition + vector_rayDirection * bounce_distanceToIntersection;
                        ...
                        // Apply a local "NdotL" term with the given ray direction.
                        bounceLightSample *= max(0.0, dot(vector_normalDirection, vector_rayDirection));
